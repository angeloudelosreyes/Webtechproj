        ...
        if($request->category == 'Individual') {
            ...
            if($results->exists()) {
                ...
                foreach($query as $data) {
                    $query = DB::table('users_shareable_files')->where(['recipient_id' => $data->id,'users_folder_files_id' => Crypt::decryptString($request->users_folder_files_id)])->exists();
                    if(!$query) {
            ...
            foreach($query as $data) {
                $query = DB::table('users_shareable_files')->where(['recipient_id' => $data->id])->exists();
                if(!$query) {
                    Mail::to($data->email)->send(new Notification($data->email));
                    DB::table('users_shareable_files')->insert([
                        'users_id'              => auth()->user()->id,
